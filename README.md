...
- Dev workflow (recommended for UI development): run Go backend and Flutter Web
  UI separately. This gives you fast hot-reload on frontend without rebuilding
  and re-embedding `_web` on every change.
  ...
  - If you previously opened UI from `network-debugger-web`, your browser may
    have cached an old `base_url`. In that case clear site data for localhost.
...
- Dev note (important): `cmd/network-debugger-web` serves the Web UI from embedded
  artifacts in `cmd/network-debugger-web/_web`. If you change the frontend (or
  you see an outdated UI / "No connection to backend"), rebuild the frontend and
  copy the build output before running `go run ./cmd/network-debugger-web`:
