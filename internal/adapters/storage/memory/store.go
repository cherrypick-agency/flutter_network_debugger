...
func (s *Store) ListSessions(ctx context.Context, f usecase.SessionFilter) ([]domain.Session, int, error) {
    ...
    for _, id := range s.order { // preserve insertion order
        ...
        if f.Target != "" && !containsFold(e.session.Target, f.Target) { continue }
        // text search best-effort in target
        if f.Q != "" && !strings.Contains(strings.ToLower(e.session.Target), strings.ToLower(f.Q)) { continue }
...
func (s *Store) ListFrames(ctx context.Context, sessionID string, from string, limit int) ([]domain.Frame, string, error) {
    ...
    if from != "" {
        ...
        for i := range e.frames {
            if e.frames[i].ID == from { start = i+1; break }
        }
    }
    end := start + limit
    if limit <= 0 || end > len(e.frames) { end = len(e.frames) }
    next := ""
    if end < len(e.frames) {
...
func (s *Store) ListEvents(ctx context.Context, sessionID string, from string, limit int) ([]domain.Event, string, error) {
    ...
    if from != "" {
        for i := range e.events {
            if e.events[i].ID == from { start = i+1; break }
        }
    }
    end := start + limit
    if limit <= 0 || end > len(e.events) { end = len(e.events) }
    next := ""
    if end < len(e.events) {
...
func (s *Store) ListHTTPTransactions(ctx context.Context, sessionID string, from string, limit int) ([]domain.HTTPTransaction, string, error) {
    ...
    if from != "" {
        for i := range e.httpTxs {
            if e.httpTxs[i].ID == from { start = i+1; break }
        }
    }
    end := start + limit
    if limit <= 0 || end > len(e.httpTxs) { end = len(e.httpTxs) }
    next := ""
    if end < len(e.httpTxs) { next = e.httpTxs[end-1].ID }
