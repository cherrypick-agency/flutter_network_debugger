...
func TestPythonCompiler_ValidateSyntax_ValidCode(t *testing.T) {
	...
	err := compiler.ValidateSyntax(ctx, req)
	if err != nil {
		t.Errorf("ValidateSyntax() error = %v, want nil for valid code", err)
	}
}

// ========== Kotlin Compiler Tests ==========

func TestKotlinCompiler_ValidateDependencies(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewKotlinCompiler(cache)

	// Валидные зависимости
	validDeps := map[string]string{
		"Utils.kt":         "class Utils {}",
...
func TestKotlinCompiler_ParseKotlinError(t *testing.T) {
	...
	if compErr.Line != 10 {
		t.Errorf("Line = %d, want 10", compErr.Line)
	}

	if compErr.Column != 5 {
		t.Errorf("Column = %d, want 5", compErr.Column)
	}
}

// ========== Swift Compiler Tests ==========

func TestSwiftCompiler_ValidateDependencies(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewSwiftCompiler(cache)

	// Валидные зависимости
	validDeps := map[string]string{
		"Utils.swift":   "class Utils {}",
	...
	// Пустой Package.swift
	emptyPackageSwift := map[string]string{
		"Package.swift": "",
	}

	err = compiler.ValidateDependencies(emptyPackageSwift)
	if err == nil {
...
func TestSwiftCompiler_GenerateMinimalPackageSwift(t *testing.T) {
	...
	if !contains(pkgSwift, "main") {
		t.Error("Package.swift should contain 'main'")
	}
}

// ========== Zig Compiler Tests ==========

func TestZigCompiler_ValidateDependencies(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewZigCompiler(cache)

	// Валидные зависимости
	validDeps := map[string]string{
		"utils.zig": "pub fn test() void {}",
...
func TestZigCompiler_ParseZigError(t *testing.T) {
	...
	output := "main.zig:10:5: error: use of undeclared identifier 'foo'"
...
10 |     let x: i32 = "hello";
  |         ^ expected i32, found &str`
	...
	if compErr.Code != "E0308" {
		t.Errorf("Code = %q, want E0308", compErr.Code)
	}
}

func TestRustCompiler_ParseRustError_SimpleError(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewRustCompiler(cache)
...
func TestRustCompiler_ValidateDependencies_MissingCdylib(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewRustCompiler(cache)

	// Cargo.toml без cdylib
	invalidDeps := map[string]string{
		"Cargo.toml": "[package]\nname = \"test\"\nversion = \"0.1.0\"",
	}
	...
	err := compiler.ValidateDependencies(invalidDeps)
	if err == nil {
		t.Error("ValidateDependencies() should return error for missing cdylib")
	}
}

// ========== TinyGo Compiler Edge Cases ==========

func TestTinyGoCompiler_ParseCompilationError_MultipleErrors(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewTinyGoCompiler(cache)

	output := `main.go:5:2: undefined: foo
main.go:6:1: syntax error: unexpected semicolon, expecting expression
main.go:10:3: undefined: bar`
	err := compiler.parseCompilationError("compilation failed", output)
...
func TestTinyGoCompiler_ParseCompilationError_NoColon(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewTinyGoCompiler(cache)

	output := "some error message without colon"
	err := compiler.parseCompilationError("compilation failed", output)
...
func TestTinyGoCompiler_GenerateMinimalGoMod_Content(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewTinyGoCompiler(cache)

	ws, err := NewWorkspace("test-gomod-content")
	if err != nil {
		t.Fatalf("NewWorkspace failed: %v", err)
	}
	defer ws.Cleanup()
	...
	if !contains(contentStr, "extism/go-pdk") {
		t.Error("go.mod should contain 'extism/go-pdk'")
	}
}

func TestTinyGoCompiler_ValidateDependencies_WithGoSum(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewTinyGoCompiler(cache)

	// go.mod с go.sum (go.sum игнорируется в валидации)
	validDeps := map[string]string{
		"go.mod": "module test\n\ngo 1.21",
		"go.sum": "some checksum content",
	}
...
func TestTinyGoCompiler_ValidateDependencies_AdditionalGoFiles(t *testing.T) {
	cache := &mockCacheManager{}
	compiler := NewTinyGoCompiler(cache)

	// go.mod с дополнительными .go файлами
	validDeps := map[string]string{
		"go.mod":   "module test\n\ngo 1.21",
