...
// ConfigRepo - реализация IConfigRepository для GORM/SQLite
type ConfigRepo struct {
	db *gorm.DB
}

// NewConfigRepo - создать новый репозиторий конфигурации
func NewConfigRepo(db *gorm.DB) *ConfigRepo {
	return &ConfigRepo{db: db}
}

// Load - загрузить конфигурацию из базы данных (singleton с ID=1)
func (r *ConfigRepo) Load(ctx context.Context) (*domain.DetectionConfig, error) {
	...
	if err != nil {
		// Если запись не найдена, создать дефолтную
...
// toDomainConfig - конвертация GORM модели в domain entity
...
// toModel - конвертация domain entity в GORM модель
