...
// PrinterApp — координатор: подписка на события, догрузка данных, печать.
	...
	for {
		...
		case ev, ok := <-ch:
			...
			case ports.EventHTTPTxAdded:
				// Пытаемся получить транзакцию точечно по ID (in-memory быстрый путь)
				...
				// Fallback: берём небольшой «хвост» и ищем нужный ID
				...
				if !found {
					// если не нашли в хвосте — не шумим, ждём следующий ивент
					continue
				}
				// Фильтрация (простая подстрока по URL/методу/статусу)
				...
				_ = p.Rend.RenderHTTP(w, vm, opts)
			default:
				// игнорируем остальные события для MVP
			}
		}
	}
}
