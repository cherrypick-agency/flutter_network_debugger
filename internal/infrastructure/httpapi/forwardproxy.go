...
func (d *Deps) handleHTTPForwardRequest(w http.ResponseWriter, r *http.Request) {
    ...
    if ip := clientHost(r.RemoteAddr); ip != "" { outReq.Header.Set("X-Forwarded-For", ip) }
    if r.TLS != nil { outReq.Header.Set("X-Forwarded-Proto", "https") } else { outReq.Header.Set("X-Forwarded-Proto", "http") }
    outReq.Header.Set("Via", "go-proxy")
