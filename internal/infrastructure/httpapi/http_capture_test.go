...
func TestV1Capture_GetStartStopAndList(t *testing.T) {
    ...
    h.ServeHTTP(rr, req)
    if rr.Code != 200 { t.Fatalf("captures list") }
    var body map[string]any
    _ = json.Unmarshal(rr.Body.Bytes(), &body)
    if _, ok := body["items"].([]any); !ok { t.Fatalf("captures items") }
    // POST stop
    rr = httptest.NewRecorder()
    req = httptest.NewRequest(http.MethodPost, "/_api/v1/capture", bytes.NewReader([]byte(`{"action":"stop"}`)))
    h.ServeHTTP(rr, req)
    if rr.Code != 200 { t.Fatalf("capture stop") }
}
...
func TestV1Capture_BadAction(t *testing.T) {
    ...
    h.ServeHTTP(rr, req)
    if rr.Code != http.StatusBadRequest { t.Fatalf("bad action -> 400") }
}
