...
func TestForwardOneWSFrame_TextMaskedPreview(t *testing.T) {
	...
	if op != domain.OpcodeText || size != 2 || prev != "Hi" {
		t.Fatalf("unexpected: %v %d %q", op, size, prev)
	}
	if !bytes.Equal(w.Bytes(), data) {
		t.Fatalf("must forward raw bytes unchanged")
	}
}
...
func TestForwardOneWSFrame_ExtendedLen126(t *testing.T) {
	...
	if op != domain.OpcodeText || size != 128 || len(prev) != 128 {
		t.Fatalf("extended len failed: %d %d", size, len(prev))
	}
}
...
func TestForwardOneWSFrame_ExtendedLen127(t *testing.T) {
	...
	if op != domain.OpcodeText || size != n || len(prev) != n {
		t.Fatalf("extended len127 failed: %d %d", size, len(prev))
	}
}
...
func TestForwardOneWSMessage_CaptureBodiesToFile(t *testing.T) {
	...
	if file == "" {
		t.Fatalf("expected body file path")
	}
}
