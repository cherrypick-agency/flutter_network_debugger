...
func TestHTTPProxy_V1RouteAlias(t *testing.T) {
    ...
    h.ServeHTTP(rr, req)
    if rr.Code != 200 { t.Fatalf("v1 httpproxy alias status: %d", rr.Code) }
}
...
func TestUnifiedProxy_HTTPPath(t *testing.T) {
    ...
    q := req.URL.Query(); q.Set("_target", upstream.URL); req.URL.RawQuery = q.Encode()
    h.ServeHTTP(rr, req)
    if rr.Code != 200 { t.Fatalf("unified proxy status: %d", rr.Code) }
}
