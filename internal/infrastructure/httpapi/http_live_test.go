...
func TestHandleWSSendText_Errors(t *testing.T) {
    ...
    if rr.Code != http.StatusServiceUnavailable { t.Fatalf("no live -> 503 expected") }

    d.Live = NewLiveSessions()
    rr = httptest.NewRecorder()
    req = httptest.NewRequest(http.MethodPost, "/api/sessions/abc/ws/send", bytes.NewReader([]byte(`{"direction":"client->upstream","payload":"hi"}`)))
    d.handleWSSendText(rr, req)
    if rr.Code != http.StatusBadRequest { t.Fatalf("unregistered session -> 400 expected, got %d", rr.Code) }
}
...
func TestHandleV1MITMGenerate(t *testing.T) {
    ...
    if rr.Code != http.StatusOK { t.Fatalf("generate status: %d", rr.Code) }
    if d.MITM == nil || d.MITM.CA == nil { t.Fatalf("CA not set in deps") }
}
