...
func TestHandleV1Settings_GetAndPost(t *testing.T) {
    ...
    if rr.Code != http.StatusOK { t.Fatalf("GET status") }

    // POST bad json
    rr = httptest.NewRecorder()
    req = httptest.NewRequest(http.MethodPost, "/api/v1/settings", bytes.NewReader([]byte("{")))
    d.handleV1Settings(rr, req)
    if rr.Code != http.StatusBadRequest { t.Fatalf("POST bad json status") }
    ...
    d.handleV1Settings(rr, req)
    if rr.Code != http.StatusOK { t.Fatalf("POST ok status") }
    if d.Cfg.ResponseDelayMinMs != 10 || d.Cfg.ResponseDelayMaxMs != 20 { t.Fatalf("cfg not set: %+v", d.Cfg) }
}
...
func TestHandleV1Settings_MethodNotAllowed(t *testing.T) {
    ...
    d.handleV1Settings(rr, req)
    if rr.Code != http.StatusMethodNotAllowed { t.Fatalf("expected 405") }
}
...
func TestHandleV1Settings_BadValue(t *testing.T) {
    ...
    d.handleV1Settings(rr, req)
    if rr.Code != http.StatusBadRequest { t.Fatalf("bad value must be 400") }
}
...
func TestHandleV1Settings_RangeSwap(t *testing.T) {
    ...
    d.handleV1Settings(rr, req)
    if rr.Code != http.StatusOK { t.Fatalf("range swap should be ok") }
}
