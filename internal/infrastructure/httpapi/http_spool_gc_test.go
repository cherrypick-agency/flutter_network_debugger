...
func TestCleanupSpoolDir_RemovesOldFiles(t *testing.T) {
    ...
    if _, err := os.Stat(old); !os.IsNotExist(err) { t.Fatalf("old file must be removed") }
    if _, err := os.Stat(recent); err != nil { t.Fatalf("recent file must remain: %v", err) }
    if _, err := os.Stat(other); err != nil { t.Fatalf("foreign file must remain: %v", err) }
}
