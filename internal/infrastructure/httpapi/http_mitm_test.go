...
func TestMITM_ShouldIntercept(t *testing.T) {
    ...
    if !m.shouldIntercept("api.ok") { t.Fatalf("allow should pass") }
    if m.shouldIntercept("api.bad") { t.Fatalf("deny by default when allow present") }
    m = &MITM{CA: &CertAuthority{}, DenySuffix: []string{".bad"}}
    if m.shouldIntercept("x.bad") { t.Fatalf("deny suffix should block") }
    if !m.shouldIntercept("x.good") { t.Fatalf("no allow -> allow all") }
}
...
func TestPEMEncodeCert_WritesPEM(t *testing.T) {
    ...
    if err != nil { t.Skip("skip crypto: ", err) }
    block, _ := pem.Decode(certPEM)
    if block == nil { t.Fatalf("no pem block") }
    if _, err := x509.ParseCertificate(block.Bytes); err != nil { t.Fatalf("parse cert: %v", err) }
    rr := httptest.NewRecorder()
    if err := pemEncodeCert(rr, block.Bytes); err != nil { t.Fatalf("pemEncodeCert: %v", err) }
    if !bytes.Contains(rr.Body.Bytes(), []byte("END CERTIFICATE")) { t.Fatalf("pem tail missing") }
}
...
func TestWriteRaw_WritesHTTPResponse(t *testing.T) {
    ...
    if err := writeRaw(&buf, resp); err != nil { t.Fatalf("writeRaw: %v", err) }
    if !bytes.Contains(buf.Bytes(), []byte("HTTP/1.1 200 OK")) { t.Fatalf("status line missing: %q", buf.String()) }
}
