...
func TestV1Session_ViewEnrichment_HTTPMetaAndSizes(t *testing.T) {
    ...
    h.ServeHTTP(rr, req)
    if rr.Code != 200 { t.Fatalf("v1 get enriched") }
    var body map[string]any
    _ = json.Unmarshal(rr.Body.Bytes(), &body)
    if _, ok := body["httpMeta"].(map[string]any); !ok { t.Fatalf("httpMeta expected") }
    if sz, ok := body["sizes"].(map[string]any); !ok || int(sz["requestBytes"].(float64)) != 10 { t.Fatalf("sizes expected: %+v", sz) }
}
...
func TestV1Session_ViewEnrichment_CORSMethodReason(t *testing.T) {
    ...
    req := httptest.NewRequest(http.MethodGet, "/_api/v1/sessions/"+sid, nil)
    h.ServeHTTP(rr, req)
    if rr.Code != 200 { t.Fatalf("v1 get enriched cors") }
}
