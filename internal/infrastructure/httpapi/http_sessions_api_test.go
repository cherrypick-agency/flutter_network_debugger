...
func (apiStubRepo) GetSession(context.Context, string) (domain.Session, bool, error) {
	return domain.Session{ID: "s1", StartedAt: time.Unix(0, 0).UTC()}, true, nil
}
func (apiStubRepo) DeleteSession(context.Context, string) error { return nil }
func (apiStubRepo) ListSessions(context.Context, uc.SessionFilter) ([]domain.Session, int, error) {
	return []domain.Session{{ID: "s1"}, {ID: "s2"}}, 2, nil
}
...
func (apiStubRepo) ListEvents(context.Context, string, string, int) ([]domain.Event, string, error) {
	return []domain.Event{{ID: "e1", Name: "ev"}}, "", nil
}

// http tx
func (apiStubRepo) AppendHTTPTransaction(context.Context, domain.HTTPTransaction) error { return nil }
func (apiStubRepo) ListHTTPTransactions(context.Context, string, string, int) ([]domain.HTTPTransaction, string, error) {
	return []domain.HTTPTransaction{{Method: "GET", URL: "u", Status: 200}}, "", nil
}
...
func TestHandleSessionByID_ExportGzipAndHar(t *testing.T) {
	...
	h.ServeHTTP(rr, req)
	if rr.Code != 200 {
		t.Fatalf("export gzip")
	}
	if rr.Header().Get("Content-Encoding") != "gzip" {
		t.Fatalf("expect gzip header")
	}
	...
	h.ServeHTTP(rr, req)
	if rr.Header().Get("Content-Encoding") != "gzip" {
		t.Fatalf("accept-encoding gzip path")
	}
}
...
func TestV1Session_BodyNotImplemented(t *testing.T) {
	...
	if rr.Code != http.StatusNotFound {
		t.Fatalf("body should be 404 not_implemented")
	}
}
...
func TestLegacySession_UnknownSubresource_404(t *testing.T) {
	...
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusNotFound {
		t.Fatalf("legacy unknown subresource should be 404")
	}
}
