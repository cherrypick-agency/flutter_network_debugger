...
func (sseStubSessions) GetSession(context.Context, string) (domain.Session, bool, error) { return domain.Session{}, false, nil }
func (sseStubSessions) DeleteSession(context.Context, string) error { return nil }
func (sseStubSessions) ListSessions(context.Context, uc.SessionFilter) ([]domain.Session, int, error) { return nil, 0, nil }
...
func (sseStubSessions) AppendHTTPTransaction(context.Context, domain.HTTPTransaction) error { return nil }
...
func TestHandleSessionStream_FanOutFrameEvent(t *testing.T) {
    ...
    s := rec.String()
    if !strContains(s, "event: frames") { t.Fatalf("expected frames SSE after broadcast: %q", s) }
}
