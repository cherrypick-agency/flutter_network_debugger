...
func (d *Deps) handleWSProxy(w http.ResponseWriter, r *http.Request) {
    ...
    if sp := r.Header.Get("Sec-WebSocket-Protocol"); sp != "" { hdr.Set("Sec-WebSocket-Protocol", sp) }
    // Some upstreams require Origin; if none from client, synthesize from target host
    if hdr.Get("Origin") == "" {
        origin := "http://" + u.Host
        if u.Scheme == "wss" { origin = "https://" + u.Host }
    ...
    if d.Live != nil { d.Live.Register(sessionID, clientConn, upstreamConn) }
    ...
    for {
        ...
        if opcode == domain.OpcodeText {
            ...
            if nsp, ev, argsJSON, ok := sio.ParseEvent(raw); ok {
                var ack *int64
                if a := tryExtractAckID(raw); a >= 0 { ack = &a }
...
func buildPreview(op domain.Opcode, data []byte) string {
    if op == domain.OpcodeText {
        ...
        if json.Unmarshal(data[:max], &js) == nil {
            ...
            if max > 0 && len(redacted) > max { redacted = redacted[:max] }
            return redacted
        }
...
func (d *Deps) handleWSSendText(w http.ResponseWriter, r *http.Request) {
    if r.Method != http.MethodPost {
        writeError(w, http.StatusMethodNotAllowed, "METHOD_NOT_ALLOWED", "use POST", nil); return
    }
    path := strings.TrimPrefix(r.URL.Path, "/api/sessions/")
    parts := strings.Split(path, "/")
    if len(parts) < 3 || parts[1] != "ws" || parts[2] != "send" {
        writeError(w, http.StatusNotFound, "NOT_FOUND", "resource not found", nil); return
    }
    id := parts[0]
    var req struct {
    ...
    if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
        writeError(w, http.StatusBadRequest, "BAD_JSON", "invalid json", nil); return
    }
    if d.Live == nil { writeError(w, http.StatusServiceUnavailable, "LIVE_UNAVAILABLE", "live not ready", nil); return }
    if err := d.Live.SendText(id, req.Direction, req.Payload); err != nil {
        writeError(w, http.StatusBadRequest, "SEND_FAILED", err.Error(), nil); return
    }
