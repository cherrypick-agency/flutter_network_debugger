...
func TestHTTPResponsePreview_Gzip_Deflate_MaskAndTruncate(t *testing.T) {
    ...
    for _, tc := range cases {
        t.Run(tc.name, func(t *testing.T) {
            ...
            if body == "" { t.Fatalf("body must be present") }
            if body == string(payload) { t.Fatalf("should be redacted, got raw json") }
...
func TestHTTPRequestPreview_HeadersRedaction_Toggle(t *testing.T) {
    ...
    if _, ok := m["headersRaw"]; ok { t.Fatalf("headersRaw must be absent when expose=false") }
    ...
    if _, ok := m2["headersRaw"]; !ok { t.Fatalf("headersRaw must be present when expose=true") }
...
func TestHTTPResponsePreview_HeadersRedaction_Toggle(t *testing.T) {
    ...
    if _, ok := m2["headersRaw"]; !ok { t.Fatalf("headersRaw must be present when expose=true") }
    raw := m2["headersRaw"].(map[string]any)
    if raw["Authorization"].(string) != "Bearer xyz" { t.Fatalf("raw authorization expected") }
}
...
func TestBuildHTTPRequestPreview_FormURLEncoded_Truncation(t *testing.T) {
    ...
    var m map[string]any
    _ = json.Unmarshal([]byte(out), &m)
    if len(m["body"].(string)) > 8 { t.Fatalf("truncation expected") }
}
...
func TestBuildHTTPRequestPreview_FormParsingAndBody(t *testing.T) {
    ...
    if _, ok := m1["form"].(map[string]any); !ok { t.Fatalf("expected form section for urlencoded") }
    ...
    var m2 map[string]any
    _ = json.Unmarshal([]byte(out2), &m2)
    if _, ok := m2["form"].(map[string]any); !ok { t.Fatalf("expected form section for multipart") }
}
...
func TestAugmentPreviewWithTimings_andHelpers(t *testing.T) {
    ...
    if int(tm["ttfbMs"].(float64)) != 10 || int(tm["totalMs"].(float64)) != 20 { t.Fatalf("timings not injected: %+v", tm) }

    // binary preview limit
    big := bytes.Repeat([]byte{0xAB}, 300)
    s := formatBinaryPreview(big, 300)
    if len(s) > 3*256 { t.Fatalf("binary preview should cap at 256 bytes") }

    // tls mappers
    if tlsVersionString(tls.VersionTLS12) == "" { t.Fatalf("tls version mapping") }
    if cipherSuiteString(tls.TLS_AES_128_GCM_SHA256) == "" { t.Fatalf("cipher mapping") }

    // time helpers
    now := time.Now()
    if durationMs(time.Time{}, now) != 0 { t.Fatalf("durationMs zero handling") }
    if useOrFallback(time.Time{}, now).IsZero() { t.Fatalf("useOrFallback") }
    if !timeFromUnixNanoOrZero(0).IsZero() { t.Fatalf("timeFromUnixNanoOrZero") }
    if int64ToInt(-1) != 0 { t.Fatalf("int64ToInt negative to zero") }
}
...
func TestRemoveHopHeaders_AndAbsoluteURL(t *testing.T) {
    ...
    h.Set("X-Ok", "1")
    removeHopHeaders(h)
    if h.Get("X-Ok") != "1" { t.Fatalf("non-hop header should remain") }
...
func TestCertsSummary_AndTryCompactJSON(t *testing.T) {
    // certsSummary empty
    if certsSummary(nil) != nil { t.Fatalf("nil -> nil") }
    c := &x509.Certificate{Subject: pkix.Name{CommonName: "cn"}}
    s := certsSummary([]*x509.Certificate{c})
    if len(s) != 1 { t.Fatalf("one cert expected") }

    if tryCompactJSON([]byte("not-json")) != "" { t.Fatalf("invalid json must return empty") }
    if tryCompactJSON([]byte("{\n \"a\" : 1 }")) != "{\"a\":1}" { t.Fatalf("should compact") }
}
...
func TestSanitizeForUpgrade_AndCookieSummary(t *testing.T) {
    ...
    if h.Get("Upgrade") == "" || h.Get("Connection") == "" { t.Fatalf("must keep upgrade/connection") }
    if h.Get("Proxy-Connection") != "" || h.Get("Keep-Alive") != "" || h.Get("Te") != "" { t.Fatalf("must strip hop headers: %+v", h) }
...
func TestHTTPResponsePreview_TLS_Summary(t *testing.T) {
    ...
    var m map[string]any
    _ = json.Unmarshal([]byte(prev), &m)
    if _, ok := m["tls"].(map[string]any); !ok { t.Fatalf("tls summary expected") }
}
...
func TestHTTPRequestPreview_GzipBody_Decompress(t *testing.T) {
    ...
    var m map[string]any
    _ = json.Unmarshal([]byte(out), &m)
    if m["body"].(string) != "{\"a\":1}" { t.Fatalf("expected decompressed json body: %v", m["body"]) }
}
...
func TestHTTPRequestPreview_NoDecompress_ShowsRaw(t *testing.T) {
    ...
    if m["body"].(string) == "{\"a\":1}" { t.Fatalf("expected raw when previewDecompress=false") }
}
...
func TestCipherSuiteString_DefaultAndInt64ToIntOverflow(t *testing.T) {
    if cipherSuiteString(0xFFFF) == "" { t.Fatalf("default cipher mapping expected non-empty") }
    if int64ToInt((1<<62)) <= 0 { t.Fatalf("overflow clamp expected >0") }
}

func TestTLSVersionString_DefaultEmpty(t *testing.T) {
    if tlsVersionString(0) != "" { t.Fatalf("unknown should map to empty string") }
    if tlsVersionString(tls.VersionTLS13) == "" { t.Fatalf("tls1.3 mapping expected") }
    if cipherSuiteString(tls.TLS_CHACHA20_POLY1305_SHA256) == "" { t.Fatalf("cipher mapping branch") }
}
...
func TestSpoolBody_WritesLimitedBytes(t *testing.T) {
    ...
    if err != nil || path == "" { t.Fatalf("spool error: %v path=%q", err, path) }
    st, err := os.Stat(path)
    if err != nil { t.Fatalf("stat: %v", err) }
    if st.Size() != 100 { t.Fatalf("expected 100 bytes spooled, got %d", st.Size()) }
    _ = os.Remove(path)
}
...
func TestCloneAndCopyHeader(t *testing.T) {
    ...
    dst := http.Header{}
    copyHeader(dst, src)
    if len(dst["A"]) != 2 { t.Fatalf("copyHeader failed: %+v", dst) }
    c := cloneHeader(dst)
    dst.Set("A", "x")
    if c.Get("A") != "1" { t.Fatalf("clone must be deep copy, got: %v", c["A"]) }
}
...
func TestPrependConn_Read(t *testing.T) {
    ...
    if err != nil || string(buf[:n]) != "HELLO WORLD" { t.Fatalf("unexpected read: n=%d err=%v data=%q", n, err, buf[:n]) }
}

func TestWriteError_JSON(t *testing.T) {
    rr := httptest.NewRecorder()
    writeError(rr, http.StatusBadRequest, "BAD", "oops", map[string]any{"x":1})
    if rr.Code != http.StatusBadRequest { t.Fatalf("status") }
    if rr.Header().Get("Content-Type") != "application/json" { t.Fatalf("content-type") }
    var body map[string]any
    _ = json.Unmarshal(rr.Body.Bytes(), &body)
    if body["error"].(map[string]any)["code"].(string) != "BAD" { t.Fatalf("code") }
}

func TestWriteError_DefaultCode(t *testing.T) {
    rr := httptest.NewRecorder()
    writeError(rr, http.StatusTeapot, "", "", nil)
    if rr.Code != http.StatusTeapot { t.Fatalf("status mismatch") }
...
func TestMultipartReaderFrom_NoBoundary(t *testing.T) {
    if mr := multipartReaderFrom("multipart/form-data", []byte("")); mr != nil { t.Fatalf("expected nil without boundary") }
}
...
func TestWithCORS_WritesHeadersAndHandlesOPTIONS(t *testing.T) {
    ...
    h.ServeHTTP(rr, req)
    if rr.Code != http.StatusNoContent { t.Fatalf("options should return 204") }
    if rr.Header().Get("Access-Control-Allow-Origin") == "" { t.Fatalf("cors headers missing") }

    // GET
    rr = httptest.NewRecorder()
    req = httptest.NewRequest(http.MethodGet, "/x", nil)
    h.ServeHTTP(rr, req)
    if rr.Code != http.StatusOK { t.Fatalf("wrapped handler should be called") }
    if rr.Header().Get("Access-Control-Allow-Methods") == "" { t.Fatalf("cors methods header missing") }
}

func TestHumanizeProxyError_Mapping(t *testing.T) {
    if c, _ := humanizeProxyError(io.EOF); c != "CONNECTION_CLOSED" { t.Fatalf("EOF mapping") }
    if c, _ := humanizeProxyError(context.DeadlineExceeded); c != "TIMEOUT" { t.Fatalf("timeout mapping") }
    if c, _ := humanizeProxyError(errors.New("connection refused")); c != "SERVER_UNAVAILABLE" { t.Fatalf("refused") }
    if c, _ := humanizeProxyError(errors.New("no such host")); c != "DNS_ERROR" { t.Fatalf("dns") }
    if c, _ := humanizeProxyError(errors.New("tls: bad cert")); c != "TLS_ERROR" { t.Fatalf("tls") }
}

func TestTryDecompress_Invalid(t *testing.T) {
    if _, ok := tryDecompress([]byte("junk"), "gzip"); ok { t.Fatalf("invalid gzip should fail") }
    if _, ok := tryDecompress([]byte("junk"), "deflate"); ok { t.Fatalf("invalid deflate should fail") }
}

func TestIsWebSocketRequest(t *testing.T) {
    r, _ := http.NewRequest(http.MethodGet, "/", nil)
    if isWebSocketRequest(r) { t.Fatalf("no ws headers -> false") }
    r.Header.Set("Sec-WebSocket-Key", "k")
    if !isWebSocketRequest(r) { t.Fatalf("sec headers -> true") }
    r2, _ := http.NewRequest(http.MethodGet, "/", nil)
    r2.Header.Set("Upgrade", "websocket")
    if !isWebSocketRequest(r2) { t.Fatalf("upgrade -> true") }
}
