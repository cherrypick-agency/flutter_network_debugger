...
func exportHARForSession(w http.ResponseWriter, r *http.Request, d *Deps, sessionID string) {
	...
	for {
		...
		if err != nil {
			writeError(w, http.StatusInternalServerError, "HTTP_LIST_FAILED", err.Error(), map[string]any{"id": sessionID})
			return
		}
		for _, tx := range txs {
	...
	har := struct {
		Log harLog `json:"log"`
	}{Log: harLog{Version: "1.2", Creator: harName{Name: "network-debugger", Version: "0.1.0"}, Entries: entries}}
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Content-Disposition", "attachment; filename=network-debugger_session_"+sessionID+".har")
