...
func TestHTTPReverseProxy_BasicGetAndPost(t *testing.T) {
    ...
    if err != nil { t.Fatalf("get via reverse: %v", err) }
    defer resp.Body.Close()
    if resp.StatusCode != http.StatusOK { t.Fatalf("status: %d", resp.StatusCode) }
    var got map[string]any
    _ = json.NewDecoder(resp.Body).Decode(&got)
    if got["q"].(string) != "42" { t.Fatalf("query not forwarded: %v", got) }
    ...
    if err != nil { t.Fatalf("post via reverse: %v", err) }
    defer resp2.Body.Close()
    if resp2.StatusCode != http.StatusOK { t.Fatalf("status: %d", resp2.StatusCode) }
...
func TestUnifiedProxy_DefaultTarget(t *testing.T) {
    ...
    if err != nil { t.Fatalf("unified proxy request: %v", err) }
    if resp.StatusCode != http.StatusOK { t.Fatalf("status: %d", resp.StatusCode) }
}
...
func TestForwardProxy_HTTP_AbsoluteURI(t *testing.T) {
    ...
    if _, err := conn.Write([]byte(raw)); err != nil { t.Fatalf("write: %v", err) }
    br := bufio.NewReader(conn)
    line, err := br.ReadString('\n')
    if err != nil { t.Fatalf("read: %v", err) }
    if line == "" { t.Fatalf("no response from proxy forward handler") }
}
...
func TestForwardProxy_CONNECT_TunnelToHTTP(t *testing.T) {
    ...
    if _, err := conn.Write([]byte(req)); err != nil { t.Fatalf("write connect: %v", err) }
    br := bufio.NewReader(conn)
    status, _ := br.ReadString('\n')
    if status == "" || status[:12] != "HTTP/1.1 200" { t.Fatalf("connect status: %q", status) }

    // now send plain HTTP GET through the tunnel to upstream
    get := "GET /get?q=77 HTTP/1.1\r\nHost: " + target + "\r\n\r\n"
    if _, err := conn.Write([]byte(get)); err != nil { t.Fatalf("tunnel write: %v", err) }
    line, _ := br.ReadString('\n')
    if line == "" { t.Fatalf("no response through tunnel") }
}
...
func TestHTTPReverseProxy_RedactionAndFrames(t *testing.T) {
    ...
    if err != nil { t.Fatalf("reverse get: %v", err) }
    resp.Body.Close()
    ...
    defer r.Body.Close()
    var list struct{ Items []struct{ ID string; Target string } `json:"items"` }
    ...
    for _, it := range list.Items {
        if strings.Contains(it.Target, "/get") { sidGet = it.ID }
        if strings.Contains(it.Target, "/gzip") { sidGzip = it.ID }
    }
    if sidGet == "" || sidGzip == "" { t.Fatalf("expected sessions for /get and /gzip; got %+v", list.Items) }
    ...
    var frames1 struct{ Items []struct{ Preview string } `json:"items"` }
    ...
    for _, f := range frames1.Items {
        ...
        if js["type"] == "http_request" {
            if hdr, ok := js["headers"].(map[string]any); ok { if hdr["Authorization"] == "***" || hdr["authorization"] == "***" { sawReqRedacted = true } }
        }
        if js["type"] == "http_response" {
            if hdr, ok := js["headers"].(map[string]any); ok { if hdr["Set-Cookie"] == "***" || hdr["set-cookie"] == "***" { sawRespRedacted = true } }
        }
    }
    if !sawReqRedacted || !sawRespRedacted { t.Fatalf("expected header redaction: req=%v resp=%v", sawReqRedacted, sawRespRedacted) }
    ...
    var frames2 struct{ Items []struct{ Preview string } `json:"items"` }
    ...
    for _, f := range frames2.Items {
        ...
        _ = json.Unmarshal([]byte(f.Preview), &js)
        if js["type"] == "http_response" {
            if body, ok := js["body"].(string); ok && len(body) >= 100 { sawBig = true }
        }
    }
    if !sawBig { t.Fatalf("expected big response body preview in /gzip session") }
    _ = deps
}
