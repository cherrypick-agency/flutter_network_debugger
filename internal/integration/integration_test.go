...
func startEchoWSServer(t *testing.T) (*httptest.Server, string) {
    ...
    mux.HandleFunc("/ws", func(w http.ResponseWriter, r *http.Request) {
        ...
        respHdr := http.Header{}
        if sp != "" { respHdr.Set("Sec-WebSocket-Protocol", sp) }
...
func TestWSProxy_EndToEnd_SessionFramesEventsAndAPI(t *testing.T) {
    ...
    for time.Now().Before(deadline) {
        select {
        case ev := <-monCh:
            if ev.Type == "frame_added" { framesCount++ }
            if ev.Type == "event_added" { eventsCount++ }
...
started:
    ...
    var framesOut struct{ Items []struct{ Opcode string `json:"opcode"`; Preview string `json:"preview"` } `json:"items"` }
    ...
    for _, f := range framesOut.Items {
        if f.Preview == "hello" { sawHello = true }
        if strings.HasPrefix(f.Preview, "42/") && strings.Contains(f.Preview, "[\"ack\"") { sawSIO = true }
    }
    if !sawHello {
    ...
    var eventsOut struct{ Items []struct{ Namespace string `json:"namespace"`; Name string `json:"event"`; AckID *int64 `json:"ackId"`; ArgsPreview string `json:"argsPreview"` } `json:"items"` }
...
func TestHeadersAndSubprotocolForwarding(t *testing.T) {
    ...
    if err := json.Unmarshal(data, &got); err != nil { t.Fatalf("json: %v", err) }
...
func TestBinaryCounters(t *testing.T) {
    ...
    // send binary
    if err := c.WriteMessage(websocket.BinaryMessage, []byte{0x01, 0x02}); err != nil { t.Fatalf("bin: %v", err) }
    ...
    defer resp.Body.Close()
    var list struct{ Items []struct{ ID string; Frames struct{ Total, Binary, Control int } } `json:"items"` }
    _ = json.NewDecoder(resp.Body).Decode(&list)
    ok := false
    for _, it := range list.Items {
        if it.Frames.Total >= 1 && it.Frames.Binary >= 1 { ok = true; break }
    }
    if !ok { t.Fatalf("counters not updated: %+v", list.Items) }
}
...
func TestPaginationFramesAndEvents(t *testing.T) {
    ...
    for {
        ...
        if next != "" { url += "&from="+next }
        r, _ := appSrv.Client().Get(url)
        var page struct{ Items []struct{ ID string } `json:"items"`; Next string `json:"next"` }
    ...
    if total < 5 { t.Fatalf("pagination frames total=%d", total) }
    ...
    if total < 5 { t.Fatalf("pagination events total=%d", total) }
}
...
func TestAPI_VersionAndCORS(t *testing.T) {
    ...
    if err != nil { t.Fatalf("version get failed: %v", err) }
    defer resp.Body.Close()
    if resp.StatusCode != http.StatusOK { t.Fatalf("status: %d", resp.StatusCode) }
    var ver map[string]any
    if err := json.NewDecoder(resp.Body).Decode(&ver); err != nil { t.Fatalf("decode failed: %v", err) }
    if ver["name"] != "go-proxy" { t.Fatalf("unexpected name: %v", ver["name"]) }
    ...
    if err != nil { t.Fatalf("preflight failed: %v", err) }
    defer resp2.Body.Close()
    if resp2.StatusCode != http.StatusNoContent { t.Fatalf("preflight status: %d", resp2.StatusCode) }
...
func TestMonitor_MultipleClients(t *testing.T) {
    ...
    for time.Now().Before(deadline) && !(got1 && got2) {
        ...
        if _, _, err := mon1.ReadMessage(); err == nil { got1 = true }
        mon2.SetReadDeadline(time.Now().Add(50 * time.Millisecond))
        if _, _, err := mon2.ReadMessage(); err == nil { got2 = true }
    }
    if !got1 || !got2 { t.Fatalf("both monitors must receive events: m1=%v m2=%v", got1, got2) }
}
...
func TestSocketIO_AckWithoutNamespace(t *testing.T) {
    ...
    defer r.Body.Close()
    var evs struct{ Items []struct{ Namespace string `json:"namespace"`; Name string `json:"event"`; AckID *int64 `json:"ackId"` } `json:"items"` }
    _ = json.NewDecoder(r.Body).Decode(&evs)
    ok := false
    for _, e := range evs.Items {
        if e.Name == "hello" && e.Namespace == "" { ok = true; break }
    }
    if !ok { t.Fatalf("expected hello event without namespace") }
}
...
func TestSessionsFilterByQuery(t *testing.T) {
    ...
    defer resp.Body.Close()
    var list struct{ Items []struct{ Target string } `json:"items"` }
    _ = json.NewDecoder(resp.Body).Decode(&list)
    found := false
    for _, it := range list.Items { if strings.Contains(it.Target, host) { found = true; break } }
    if !found { t.Fatalf("q filter did not match expected target") }
}
...
func TestCORS_Methods(t *testing.T) {
    ...
    defer resp.Body.Close()
    if !strings.Contains(resp.Header.Get("Access-Control-Allow-Headers"), "Authorization") { t.Fatalf("Authorization not allowed in CORS headers") }
    if !strings.Contains(resp.Header.Get("Access-Control-Allow-Headers"), "Cookie") { t.Fatalf("Cookie not allowed in CORS headers") }
}

// Sustained realtime connection test: keeps a wsproxy session open and exchanges frames over time
func TestSustainedRealtimeSession(t *testing.T) {
    ...
    if err != nil { t.Fatalf("monitor dial: %v", err) }
    defer mon.Close()

    // start proxy session
    c, _, err := websocket.DefaultDialer.Dial(wsURLFromHTTP(appSrv.URL, "/wsproxy")+"?target="+url.QueryEscape(echoWS), nil)
    if err != nil { t.Fatalf("proxy dial: %v", err) }
    defer c.Close()
    ...
    for i := 0; i < 5; i++ {
        ...
        if err := c.WriteMessage(websocket.TextMessage, []byte(msg)); err != nil { t.Fatalf("write: %v", err) }
        totalText++
    ...
    _ = json.NewDecoder(resp.Body).Decode(&list)
    if len(list.Items) == 0 { t.Fatalf("no sessions found") }
    ...
    if sess.Events.Total < 1 { t.Fatalf("expected at least one parsed SIO event") }
    ...
    for time.Now().Before(monDeadline) && !(hasStarted && hasEnded) {
        ...
        if _, data, err := mon.ReadMessage(); err == nil {
            ...
            if ev.Type == "session_started" { hasStarted = true }
            if ev.Type == "session_ended" { hasEnded = true }
        }
    }
    if !hasStarted || !hasEnded { t.Fatalf("monitor did not observe full lifecycle: started=%v ended=%v", hasStarted, hasEnded) }
}
...
func TestHighLoadConcurrentSessions(t *testing.T) {
    ...
    go func() {
        for {
            ...
            if _, data, err := mon.ReadMessage(); err == nil {
                var ev monitorEvent
                _ = json.Unmarshal(data, &ev)
                if ev.Type == "frame_added" { atomic.AddInt32(&monFrames, 1) }
                if ev.Type == "event_added" { atomic.AddInt32(&monEvents, 1) }
            }
        }
    }()
    ...
    for s := 0; s < sessions; s++ {
        go func(idx int) {
            defer wg.Done()
            c, _, err := websocket.DefaultDialer.Dial(wsURLFromHTTP(appSrv.URL, "/wsproxy")+"?target="+url.QueryEscape(echoWS), nil)
            if err != nil { t.Errorf("dial: %v", err); return }
            defer c.Close()
    ...
    var list struct{ Items []struct{ ID string } `json:"items"` }
    _ = json.NewDecoder(resp.Body).Decode(&list)
    if len(list.Items) == 0 { t.Fatalf("no sessions returned for target") }
    ...
    for _, f := range frames.Items {
        if strings.Contains(f.Preview, "access_token") && strings.Contains(f.Preview, "***") { redacted = true; break }
    }
    if !redacted { t.Fatalf("expected redacted access_token in preview") }

    re, _ := appSrv.Client().Get(appSrv.URL+"/api/sessions/"+sid+"/events?limit=200")
    defer re.Body.Close()
    var events struct{ Items []struct{ Namespace string; Name string } `json:"items"` }
    _ = json.NewDecoder(re.Body).Decode(&events)
    hasAdmin := false
    for _, e := range events.Items { if e.Namespace == "/admin" || e.Name == "cmd" { hasAdmin = true; break } }
    if !hasAdmin { t.Fatalf("expected admin/cmd event parsed") }
...
func TestRichServerManyEvents(t *testing.T) {
    ...
    go func() {
        ...
        for time.Now().Before(deadline) {
            ...
            var ev monitorEvent
            _ = json.Unmarshal(data, &ev)
            if ev.Type == "event_added" { hasEvent = true }
            if ev.Type == "frame_added" { hasFrame = true }
        }
    }()
    ...
    defer rf.Body.Close()
    var frames struct{ Items []struct{ Opcode string `json:"opcode"`; Preview string `json:"preview"` } `json:"items"` }
    _ = json.NewDecoder(rf.Body).Decode(&frames)
    hasBinary := false
    hasRedacted := false
    hasSrvEvent := false
    for _, f := range frames.Items {
        if strings.HasPrefix(f.Preview, "[binary ") { hasBinary = true }
        if strings.Contains(f.Preview, "access_token") && strings.Contains(f.Preview, "***") { hasRedacted = true }
        if strings.Contains(f.Preview, "\"srv_event\"") { hasSrvEvent = true }
    }
    if !hasBinary { t.Fatalf("expected at least one binary frame from server") }
    if !hasRedacted { t.Fatalf("expected redacted sensitive field in server welcome json") }
    if !hasSrvEvent { t.Fatalf("expected srv_event frame preview") }
    ...
    foundSrv := false
    for _, e := range evs.Items {
        if e.Namespace == "/chat" && e.Name == "srv_event" { foundSrv = true; break }
    }
    if !foundSrv { t.Fatalf("expected parsed srv_event in events list") }
...
func TestDeleteSessionAnd404(t *testing.T) {
    ...
    resp2.Body.Close()
    if resp2.StatusCode != http.StatusNoContent { t.Fatalf("delete status: %d", resp2.StatusCode) }
...
func TestUpstreamDialFailureSetsClosedError(t *testing.T) {
    ...
    resp, _ := appSrv.Client().Get(appSrv.URL+"/api/sessions?limit=1000")
    defer resp.Body.Close()
    var list struct{ Items []struct{ ID string; Error *string `json:"error"`; ClosedAt *time.Time `json:"closedAt"` } `json:"items"` }
    _ = json.NewDecoder(resp.Body).Decode(&list)
    // допускаем, что могла создаться и закрыться
    hasErr := false
    for _, it := range list.Items {
        if it.Error != nil || it.ClosedAt != nil { hasErr = true; break }
    }
    if !hasErr { t.Fatalf("expected at least one closed/error session") }
...
func TestListFiltersAndRedaction(t *testing.T) {
    ...
    resp, _ := appSrv.Client().Get(appSrv.URL+"/api/sessions?limit=1000&target="+url.QueryEscape(echoWS))
    defer resp.Body.Close()
    var list struct{ Items []struct{ Target string } `json:"items"`; Total int `json:"total"` }
    _ = json.NewDecoder(resp.Body).Decode(&list)
    if list.Total == 0 { t.Fatalf("filter by target returned 0") }
    ...
    if !redacted { t.Fatalf("sensitive fields not redacted in preview") }
}
