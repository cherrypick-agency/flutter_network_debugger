...
func TestSocketIO_AckWithoutNamespace(t *testing.T) {
	...
	sid := list.Items[len(list.Items)-1].ID
	r, err := appSrv.Client().Get(appSrv.URL + "/api/sessions/" + sid + "/events?limit=100")
	if err != nil {
		t.Fatal(err)
	}
	defer r.Body.Close()
...
func TestSessionsFilterByQuery(t *testing.T) {
	...
	host := u.Host
	resp, err := appSrv.Client().Get(appSrv.URL + "/api/sessions?limit=1000&q=" + url.QueryEscape(host[:3]))
	if err != nil {
		t.Fatal(err)
	}
	defer resp.Body.Close()
...
func TestDeleteSessionAnd404(t *testing.T) {
	...
	// get should 404
	resp3, err := appSrv.Client().Get(appSrv.URL + "/api/sessions/" + sid)
	if err != nil {
		t.Fatal(err)
	}
	defer resp3.Body.Close()
...
func TestListFiltersAndRedaction(t *testing.T) {
	...
	// pick latest session
	resp2, err := appSrv.Client().Get(appSrv.URL + "/api/sessions?limit=1000")
	if err != nil {
		t.Fatal(err)
	}
	defer resp2.Body.Close()
	...
	r3, err := appSrv.Client().Get(appSrv.URL + "/api/sessions/" + sid + "/frames?limit=100")
	if err != nil {
		t.Fatal(err)
	}
	defer r3.Body.Close()
