...
Forward proxy:
- CONNECT and absolute-URI supported (`withForwardProxy` wrapper)
- ws:// over forward: Upgrade 101 handled, bidirectional streaming with message-level previews (FIN/continuation reassembly)
- wss:// over forward: optional MITM (dev CA), after 101 streams with message-level previews
- Optional permessage-deflate preview (WS_DEFLATE_PREVIEW), limits via WS_PREVIEW_MAX_BYTES
- Forward WS text now best-effort parses Socket.IO events
- Sessions REST:
...
- Preview body threshold/truncation: `PREVIEW_MAX_BYTES` (default 4096)
  - WS preview can be overridden by `WS_PREVIEW_MAX_BYTES`; deflate preview via `WS_DEFLATE_PREVIEW`
