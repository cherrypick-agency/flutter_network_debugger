...
class InspectorRepositoryImpl implements InspectorRepository {
  ...
  Future<List<Session>> listSessions({String? q, String? target}) async {
    try {
      final res = await _api.get(host: null, path: '/_api/v1/sessions', query: {
        'limit': '100', if (q != null && q.isNotEmpty) 'q': q, if (target != null && target.isNotEmpty) 'target': target,
      });
      final data = (res.data is Map<String, dynamic>) ? (res.data as Map<String, dynamic>) : {};
      final items = ((data['items'] as List?) ?? const [])
          .whereType<Map<String, dynamic>>()
          .toList(growable: false);
      return items.map((m) => Session(
        id: m['id'] as String,
        target: m['target'] as String,
        clientAddr: m['clientAddr'] as String?,
        startedAt: m['startedAt'] != null ? DateTime.tryParse(m['startedAt'].toString()) : null,
  ...
  @override
  Future<List<Frame>> listFrames(String sessionId, {String? from, int limit = 100}) async {
    try {
      final res = await _api.get(path: '/_api/v1/sessions/$sessionId/frames', query: {
        'limit': '$limit', if (from != null && from.isNotEmpty) 'from': from,
      });
      final data = (res.data is Map<String, dynamic>) ? (res.data as Map<String, dynamic>) : {};
      final items = ((data['items'] as List?) ?? const [])
          .whereType<Map<String, dynamic>>()
          .toList(growable: false);
      return items.map((m) => Frame(
        id: m['id'] as String,
  ...
  @override
  Future<List<EventEntity>> listEvents(String sessionId, {String? from, int limit = 100}) async {
    try {
      final res = await _api.get(path: '/_api/v1/sessions/$sessionId/events', query: {
        'limit': '$limit', if (from != null && from.isNotEmpty) 'from': from,
      });
      final data = (res.data is Map<String, dynamic>) ? (res.data as Map<String, dynamic>) : {};
      final items = ((data['items'] as List?) ?? const [])
          .whereType<Map<String, dynamic>>()
          .toList(growable: false);
      return items.map((m) => EventEntity(
        id: m['id'] as String,
        ts: DateTime.parse(m['ts'] as String),
  ...
  Future<List<Map<String, dynamic>>> aggregateSessions({String groupBy = 'domain'}) async {
    final res = await _api.get(path: '/_api/v1/sessions/aggregate', query: {'groupBy': groupBy});
    final data = (res.data is Map<String, dynamic>) ? (res.data as Map<String, dynamic>) : {};
    final groups = (data['groups'] as List<dynamic>? ?? const []).cast<Map<String, dynamic>>();
    return groups;
  }
}
