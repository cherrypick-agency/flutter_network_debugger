...
class FramesTimelineLegend extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        dot(t.body.color ?? c.textPrimary),
        const SizedBox(width: 4),
        Text('Text', style: legendTextStyle),
        const SizedBox(width: 8),
        dot(c.warning),
        const SizedBox(width: 4),
        Text('Binary/Ping', style: legendTextStyle),
        const SizedBox(width: 8),
        dot(c.success),
        const SizedBox(width: 4),
        Text('Pong', style: legendTextStyle),
        const SizedBox(width: 8),
        dot(c.danger),
        const SizedBox(width: 4),
        Text('Close', style: legendTextStyle),
      ],
    );
  }
}
