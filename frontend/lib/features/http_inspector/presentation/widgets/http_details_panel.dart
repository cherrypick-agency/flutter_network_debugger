...
class _HttpDetailsPanelState extends State<HttpDetailsPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    final durationMs = (reqTs != null && respTs != null) ? respTs.difference(reqTs).inMilliseconds : null;
  ...
  Widget _buildRequest(BuildContext context, Map<String, dynamic>? req) {
    ...
    final headers = (req['headers'] as Map?)?.map((k, v) => MapEntry(k.toString(), v.toString())) ?? <String, String>{};
    final headersRaw = (req['headersRaw'] as Map?)?.map((k, v) => MapEntry(k.toString(), v.toString())) ?? <String, String>{};
    ...
    final cookieHeader = headersRaw.entries.firstWhere(
      (e)=> e.key.toLowerCase()=='cookie', orElse: ()=> const MapEntry('', ''),
    ).value.isNotEmpty
      ? headersRaw.entries.firstWhere((e)=> e.key.toLowerCase()=='cookie', orElse: ()=> const MapEntry('', '')).value
      : headers.entries.firstWhere((e)=> e.key.toLowerCase()=='cookie', orElse: ()=> const MapEntry('', '')).value;
    final reqCookies = _parseRequestCookies(cookieHeader);
    return ListView(
      children: [
        // Полная строка URL в отдельной строке со скроллом
        SingleChildScrollView(scrollDirection: Axis.horizontal, child:
          SelectableText('${(req['method'] ?? '').toString().toUpperCase()}  $url', style: context.appText.subtitle)),
        ...
        Wrap(spacing: 8, children: [
          ...
          TextButton.icon(
            onPressed: (){ final curl = _buildCurl(req); Clipboard.setData(ClipboardData(text: curl)); },
        ...
        if (qp.isNotEmpty) ...[
          ...
          ...qp.entries.map((e)=> Padding(
            padding: const EdgeInsets.only(bottom: 2),
            child: SelectableText('${e.key}: ${e.value.join(', ')}', style: context.appText.monospace),
          )),
          const SizedBox(height: 8),
        ],
        if (reqCookies.isNotEmpty) ...[
          ...
          const SizedBox(height: 4),
          ...reqCookies.entries.map((e)=> Padding(
            padding: const EdgeInsets.only(bottom: 2),
        ...
        const SizedBox(height: 4),
        ...headers.entries.map((e) => Padding(
          padding: const EdgeInsets.only(bottom: 2),
          child: _HeaderItem(name: e.key, value: e.value, raw: headersRaw[e.key]),
        )).toList(),
        const SizedBox(height: 8),
        if (body.isNotEmpty) Row(children: [
          Text('Body', style: context.appText.subtitle),
          const SizedBox(width: 12),
          FilterChip(
            label: const Text('Pretty'),
            selected: _prettyReq && !_treeReq,
            onSelected: (v){ setState(() { _treeReq = false; _prettyReq = true; }); },
          ),
          ...
          if (_isJson(body))
            FilterChip(
              ...
              selected: _treeReq,
              onSelected: (v){ setState(() { _treeReq = v; _prettyReq = !v; }); },
            ),
          const SizedBox(width: 8),
          TextButton.icon(onPressed: (){ Clipboard.setData(ClipboardData(text: body)); }, icon: const Icon(Icons.copy, size: 16), label: const Text('Copy')),
        ]),
  ...
  Widget _buildResponse(BuildContext context, Map<String, dynamic>? resp) {
    ...
    final headers = (resp['headers'] as Map?)?.map((k, v) => MapEntry(k.toString(), v.toString())) ?? <String, String>{};
    final headersRaw = (resp['headersRaw'] as Map?)?.map((k, v) => MapEntry(k.toString(), v.toString())) ?? <String, String>{};
    final body = (resp['body'] ?? '').toString();
    final req = _findByType(widget.frames, 'http_request');
    final url = req?['url']?.toString() ?? '';
    final reqHeaders = (req?['headers'] as Map?)?.map((k, v) => MapEntry(k.toString(), v.toString())) ?? <String, String>{};
    final reqTs = _tsOf(widget.frames, 'http_request');
    final respTs = _tsOf(widget.frames, 'http_response');
    final durationMs = (reqTs != null && respTs != null) ? respTs.difference(reqTs).inMilliseconds : null;
      ...
      children: [
        ...
        if (durationMs != null || _respTtfbMs != null || _respTotalMs != null) Padding(
          ...
          child: Wrap(spacing: 8, children: [
            ...
            if (_respTtfbMs != null) _chip(context, 'TTFB: ${_respTtfbMs} ms'),
            if (_respTotalMs != null && _respTtfbMs != null && (_respTotalMs! - _respTtfbMs!) >= 0)
              _chip(context, 'Download: ${_respTotalMs! - _respTtfbMs!} ms'),
          ]),
        ),
        const SizedBox(height: 8),
        Text('Headers', style: context.appText.subtitle),
        ...
        if (body.isNotEmpty) Row(children: [
          ...
          FilterChip(
            ...
            onSelected: (v){ setState(() { _treeResp = false; _prettyResp = true; }); },
          ),
          const SizedBox(width: 8),
          if (_isJson(body))
            FilterChip(
              ...
              selected: _treeResp,
              onSelected: (v){ setState(() { _treeResp = v; _prettyResp = !v; }); },
            ),
          const SizedBox(width: 8),
          TextButton.icon(onPressed: (){ Clipboard.setData(ClipboardData(text: body)); }, icon: const Icon(Icons.copy, size: 16), label: const Text('Copy')),
          const SizedBox(width: 8),
          if (url.isNotEmpty) _loadingFetch
            ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2))
            : IconButton(onPressed: () => _refetch(context, url), icon: const Icon(Icons.refresh, size: 16)),
        ]),
        if (body.isNotEmpty) const SizedBox(height: 6),
        if (body.isNotEmpty)
        ...
        Wrap(spacing: 8, runSpacing: 4, children: [
          if (cache['status'] != null) _chip(context, 'cache: ${cache['status']}'),
          _chip(context, cors['ok'] == true ? 'CORS OK' : 'CORS Fail'),
          if ((headers['Vary'] ?? headers['vary']) != null) _chip(context, 'Vary: ${(headers['Vary'] ?? headers['vary'])}'),
        ]),
        const SizedBox(height: 6),
        // Cache table
        if (cache.isNotEmpty) ...[
  ...
  Map<String, dynamic> _computeCacheMeta(int status, Map<String, String> respHdr) {
    final out = <String, dynamic>{};
    final cc = respHdr.entries.firstWhere((e)=> e.key.toLowerCase()=='cache-control', orElse: ()=> const MapEntry('', '')).value;
    final etag = respHdr.entries.firstWhere((e)=> e.key.toLowerCase()=='etag', orElse: ()=> const MapEntry('', '')).value;
    final ageStr = respHdr.entries.firstWhere((e)=> e.key.toLowerCase()=='age', orElse: ()=> const MapEntry('', '')).value;
    final age = int.tryParse(ageStr) ?? 0;
    String st = 'MISS';
    if (status == 304) st = 'REVALIDATED'; else if (age > 0) st = 'HIT';
    out['status'] = st;
  ...
  Map<String, dynamic> _computeCorsMeta(String method, Map<String, String> reqHdr, Map<String, String> respHdr) {
    final out = <String, dynamic>{};
    final origin = _getFold(reqHdr, 'Origin');
    final allowOrigin = _getFold(respHdr, 'Access-Control-Allow-Origin');
    final allowMethods = _splitCsv(_getFold(respHdr, 'Access-Control-Allow-Methods'));
    final allowHeaders = _splitCsv(_getFold(respHdr, 'Access-Control-Allow-Headers'));
    final isPreflight = method.toUpperCase() == 'OPTIONS' && _getFold(reqHdr, 'Access-Control-Request-Method').isNotEmpty;
    bool ok;
    String reason = '';
    if (origin.isEmpty) {
      ok = true;
    } else if (isPreflight) {
      final reqMethod = _getFold(reqHdr, 'Access-Control-Request-Method').toUpperCase();
      final reqHeaders = _splitCsv(_getFold(reqHdr, 'Access-Control-Request-Headers'));
      final originOk = (allowOrigin == '*' || allowOrigin == origin);
      final methodOk = allowMethods.isEmpty ? true : allowMethods.map((e)=> e.toUpperCase()).contains(reqMethod);
      final headersOk = reqHeaders.every((h)=> allowHeaders.any((a)=> a.toLowerCase() == h.toLowerCase()));
      ok = originOk && methodOk && headersOk;
      if (!originOk) reason = 'origin'; else if (!methodOk) reason = 'method'; else if (!headersOk) reason = 'headers';
    } else {
      final originOk = (allowOrigin == '*' || allowOrigin == origin);
      ok = originOk;
  ...
  String _getFold(Map<String, String> h, String key) {
    ...
    for (final e in h.entries) { if (e.key.toLowerCase() == lk) return e.value; }
    return '';
  }

  List<MapEntry<String, String>> _kvList(Map<String, dynamic> m) {
    return m.entries.map((e)=> MapEntry(e.key, e.value is List ? (e.value as List).join(', ') : e.value.toString())).toList();
  }

  Uri? _tryParseUri(String s) { try { return Uri.parse(s); } catch (_) { return null; } }
  ...
  List<Widget> _securityRows(Map<String, dynamic> resp, Map<String, String> headers) {
    final List<Widget> out = [];
    // TLS summary from response preview if present
    final tls = resp['tls'];
    if (tls is Map) {
      out.addAll([
        SelectableText('TLS: ${tls['version'] ?? ''} ${tls['cipherSuite'] ?? ''}  ALPN: ${tls['alpn'] ?? ''}', style: context.appText.monospace),
        if ((tls['serverName'] ?? '').toString().isNotEmpty)
          SelectableText('SNI: ${tls['serverName']}', style: context.appText.monospace),
      ]);
      final certs = (tls['peerCertificates'] as List?)?.cast<Map<String, dynamic>>() ?? const [];
      if (certs.isNotEmpty) {
        out.add(const SizedBox(height: 4));
        out.add(Text('Certificates', style: Theme.of(context).textTheme.labelLarge));
        for (final c in certs) {
          out.add(Padding(
            padding: const EdgeInsets.only(bottom: 2),
            child: SelectableText('- ${c['subject']} | Issuer: ${c['issuer']} | NotAfter: ${c['notAfter']}', style: context.appText.monospace),
          ));
        }
      }
    ...
    if (cs is Map) {
      ...
      out.add(SelectableText('Set-Cookie: ${cs['setCookieCount']} | Secure: ${cs['secure']} | HttpOnly: ${cs['httpOnly']} | SameSite Lax/Strict/None: ${cs['sameSiteLax']}/${cs['sameSiteStrict']}/${cs['sameSiteNone']}', style: context.appText.monospace));
    }
    if (out.isEmpty) out.add(SelectableText('—', style: context.appText.monospace));
    return out;
  }
  ...
  Future<void> _refetch(BuildContext context, String url) async {
    ...
    try {
      ...
      final res = await (client as dynamic).get(path: '/httpproxy', query: {'target': url});
      ...
      await showModalBottomSheet(context: context, isScrollControlled: true, builder: (_) => Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Fetched Body', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            Expanded(child: SingleChildScrollView(child: SelectableText(data, style: context.appText.monospace))),
            Align(alignment: Alignment.centerRight, child: TextButton.icon(onPressed: (){ Clipboard.setData(ClipboardData(text: data)); }, icon: const Icon(Icons.copy, size: 16), label: const Text('Copy'))),
          ],
        ),
      ));
    } catch (e) {
      ...
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${msg.title}: ${msg.description}')));
    } finally {
...
class _HeaderItemState extends State<_HeaderItem> {
  ...
  @override
  Widget build(BuildContext context) {
    final nameStyle = Theme.of(context).textTheme.bodySmall?.copyWith(fontFamily: 'monospace', fontWeight: FontWeight.w600);
    final valueStyle = Theme.of(context).textTheme.bodySmall?.copyWith(fontFamily: 'monospace');
    final iconSize = valueStyle?.fontSize ?? 12;
    final iconColor = valueStyle?.color;
    final lname = widget.name.toLowerCase();
    final isSensitive = lname == 'authorization' || lname == 'cookie' || lname == 'set-cookie' || lname.contains('token') || lname.contains('secret') || lname.contains('api-key') || lname.contains('apikey');
                        ...
                        MouseRegion(
                          ...
                          child: GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: _iconHover ? Theme.of(context).colorScheme.primary.withOpacity(0.12) : Colors.transparent,
                                borderRadius: BorderRadius.circular(4),
                              ),
                              child: Icon(_reveal ? Icons.visibility_off : Icons.visibility, size: iconSize, color: _iconHover ? Theme.of(context).colorScheme.primary : iconColor),
                            ),
                          ),
                        ),
                      MouseRegion(
                        ...
                        child: GestureDetector(
                          onTap: () {
                            Clipboard.setData(ClipboardData(text: '${widget.name}: $raw'));
                          },
                          child: Container(
                            padding: const EdgeInsets.symmetric(horizontal: 2, vertical: 1),
                            margin: const EdgeInsets.only(left: 6),
                            decoration: BoxDecoration(color: _iconHover ? Theme.of(context).colorScheme.primary.withOpacity(0.12) : Colors.transparent, borderRadius: BorderRadius.circular(4)),
                            child: Icon(Icons.copy, size: iconSize, color: _iconHover ? Theme.of(context).colorScheme.primary : iconColor),
                          ),
                        ),
                      ),
                    ]),
