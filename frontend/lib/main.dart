...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> _toggleTheme() async {
    ...
    await PrefsService().saveThemeModeString(
      _mode == ThemeMode.light ? 'light' : _mode == ThemeMode.dark ? 'dark' : 'system');
  }
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  final TextEditingController _sessionTargetFilterCtrl = TextEditingController();
  ...
  void _connectMonitor() {
    ...
    final wsUrl = _wsURL(sl<http_client.AppHttpClient>().defaultHost, '/_api/v1/monitor/ws');
    Future<void>(() async {
      try {
        final sock = await io.WebSocket.connect(wsUrl)
            .timeout(const Duration(seconds: 3));
        final ch = ws_io.IOWebSocketChannel(sock);
        setState(() { _monitor = ch; });
        _monitor!.stream.listen((msg) {
          final s = msg.toString();
          setState(() { _monitorLog.insert(0, s); });
  ...
  Future<void> _warmupHttpMeta() async {
    ...
    for (final s in list) {
      ...
      try {
        // ignore: invalid_use_of_protected_member
        final r = await (client as dynamic).get(path: '/api/sessions/$id/frames', query: {'limit': '2'});
        final m = <String, dynamic>{};
        final data = (r.data is Map<String, dynamic>) ? (r.data as Map<String, dynamic>) : {};
        ...
        for (final f in items) {
          ...
          try {
            ...
            if (mp['type'] == 'http_request') { req = mp; tReq = DateTime.tryParse(f['ts']?.toString() ?? ''); }
            if (mp['type'] == 'http_response') { resp = mp; tResp = DateTime.tryParse(f['ts']?.toString() ?? ''); }
          } catch (_) {}
        }
        if (req != null) m['method'] = (req['method'] ?? '').toString();
        if (resp != null) {
          m['status'] = (resp['status'] ?? '').toString();
          final headers = (resp['headers'] as Map?)?.map((k, v) => MapEntry(k.toString(), v.toString())) ?? {};
          final ctEntry = headers.entries.firstWhere(
            (e) => e.key.toLowerCase() == 'content-type', orElse: () => const MapEntry('',''),
          );
          m['mime'] = ctEntry.value;
          final upg = headers.entries.firstWhere(
            (e) => e.key.toLowerCase() == 'upgrade', orElse: () => const MapEntry('',''),
          ).value;
          m['streaming'] = (m['mime']?.toString().contains('text/event-stream') ?? false) || (upg.toString().toLowerCase() == 'websocket');
          m['headers'] = headers;
        }
  ...
  void _onFramesScroll() {
    if (_framesCtrl.position.pixels >= _framesCtrl.position.maxScrollExtent - 200) { context.read<SessionDetailsStore>().loadMoreFrames(); }
  }

  void _onEventsScroll() {
    if (_eventsCtrl.position.pixels >= _eventsCtrl.position.maxScrollExtent - 200) { context.read<SessionDetailsStore>().loadMoreEvents(); }
  }
  ...
  void _startAutoRefresh() {
    ...
    _pollTimer = Timer.periodic(const Duration(seconds: 2), (_) => _tickRefresh());
  }
  ...
  Future<void> _deleteSelected() async {
    ...
    // ignore: invalid_use_of_protected_member
    final client = sl.get<Object>();
    try { await (client as dynamic).delete(path: '/_api/v1/sessions/$id'); } catch (_) {}
      setState(() {
        _selectedSessionId = null; _frames.clear(); _events.clear();
      });
      await _loadSessions();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final globalHandlers = hk.buildHandlers({
      ...
      'sessions.focusSearch': (){ _searchFocus.requestFocus(); },
    ...
    return CallbackShortcuts(
      ...
      */
      body: Stack(children: [
        ...
        ConnectivityBanner(baseUrl: () => sl<http_client.AppHttpClient>().defaultHost),
        Expanded(
          child: Padding(
        ...
        child: Column(
          children: [
            ...
            Theme(
              data: Theme.of(context).copyWith(
                elevatedButtonTheme: ElevatedButtonThemeData(
                  style: ButtonStyle(
                    textStyle: MaterialStatePropertyAll(const TextStyle(fontSize: 12)),
                    padding: MaterialStatePropertyAll(const EdgeInsets.symmetric(horizontal: 12, vertical: 8)),
                  ),
                ),
                textButtonTheme: TextButtonThemeData(
                  style: ButtonStyle(
                    textStyle: MaterialStatePropertyAll(const TextStyle(fontSize: 12)),
                    padding: MaterialStatePropertyAll(const EdgeInsets.symmetric(horizontal: 8, vertical: 6)),
                  ),
                ),
              ),
              ...
              TextButton.icon(
                onPressed: (){ Navigator.of(context).pushNamed('/hotkeys'); },
                icon: const Icon(Icons.keyboard, size: 16),
                label: const Text('Hotkeys'),
              ),
            ]),
            ),
            // Waterfall timeline
            ...
            // Aggregate panel
            Observer(builder: (_) {
              final agg = context.watch<AggregateStore>();
              if (!agg.loading && agg.groups.isEmpty) {
                // lazy load on first build
                Future.microtask(()=> agg.load(groupBy: 'domain'));
              }
              return SizedBox(
                height: 32,
                child: ListView.separated(
                  ...
                  itemBuilder: (_, i) {
                    if (i >= agg.groups.length) return const SizedBox.shrink();
                    final g = agg.groups[i];
                    final key = (g['key'] ?? '').toString();
                    final selected = _selectedDomains.contains(key);
                    return ChoiceChip(
                      label: Text('$key (${g['count']})', style: const TextStyle(fontSize: 12)),
                      selected: selected,
                      visualDensity: const VisualDensity(horizontal: -3, vertical: -3),
                      materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                      onSelected: (_){
                        setState(() {
                          if (selected) { _selectedDomains.remove(key); } else { _selectedDomains.add(key); }
                        });
                      },
                    );
                  },
                  separatorBuilder: (_, __) => const SizedBox(width: 6),
                  itemCount: agg.groups.length,
                ),
              );
            }),
            if (_showFilters)
              ...
              SizedBox(
                width: 300,
                 child: TextField(style: const TextStyle(fontSize: 12), focusNode: _searchFocus, controller: _sessionSearchCtrl, decoration: const InputDecoration(labelText: 'Search sessions (q)'), onChanged: (_){ _savePrefs(); _scheduleSessionsReload(); }, onSubmitted: (_){ _savePrefs(); _loadSessions(); }),
              ),
              SizedBox(
                width: 300,
                 child: TextField(style: const TextStyle(fontSize: 12), controller: _sessionTargetFilterCtrl, decoration: const InputDecoration(labelText: 'Filter by target'), onChanged: (_){ _savePrefs(); _scheduleSessionsReload(); }, onSubmitted: (_){ _savePrefs(); _loadSessions(); }),
              ),
              ...
              DropdownButton<String>(
                ...
                items: const [
                  DropdownMenuItem(value: 'any', child: Text('Any method', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'GET', child: Text('GET', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'POST', child: Text('POST', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'PUT', child: Text('PUT', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'DELETE', child: Text('DELETE', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'PATCH', child: Text('PATCH', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'OPTIONS', child: Text('OPTIONS', style: TextStyle(fontSize: 12))),
                ],
                onChanged: (v){ setState(() { _httpMethodFilter = v ?? 'any'; }); },
              ),
              DropdownButton<String>(
                ...
                style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.onSurface),
                items: const [
                  DropdownMenuItem(value: 'any', child: Text('Any status', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: '2xx', child: Text('2xx', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: '3xx', child: Text('3xx', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: '4xx', child: Text('4xx', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: '5xx', child: Text('5xx', style: TextStyle(fontSize: 12))),
                ],
                onChanged: (v){ setState(() { _httpStatusFilter = v ?? 'any'; }); },
              ),
               SizedBox(width: 200, child: TextField(style: const TextStyle(fontSize: 12), decoration: const InputDecoration(labelText: 'MIME contains'), onChanged: (v){ setState(() { _httpMimeFilter = v; }); })),
               SizedBox(width: 120, child: TextField(style: const TextStyle(fontSize: 12), keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Min ms'), onChanged: (v){ setState(() { _httpMinDurationMs = int.tryParse(v) ?? 0; }); })),
              DropdownButton<String>(
                value: _groupBy,
                isDense: true,
                iconSize: 18,
                style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.onSurface),
                items: const [
                  DropdownMenuItem(value: 'none', child: Text('No grouping', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'domain', child: Text('Group by domain', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'route', child: Text('Group by route', style: TextStyle(fontSize: 12))),
                ],
                onChanged: (v){ setState(() { _groupBy = v ?? 'none'; }); },
              ),
               SizedBox(width: 160, child: TextField(style: const TextStyle(fontSize: 12), controller: _headerKeyCtrl, decoration: const InputDecoration(labelText: 'Header key'), onChanged: (_){ _scheduleSessionsReload(); })),
               SizedBox(width: 180, child: TextField(style: const TextStyle(fontSize: 12), controller: _headerValCtrl, decoration: const InputDecoration(labelText: 'Header value'), onChanged: (_){ _scheduleSessionsReload(); })),
              Observer(builder: (_) {
                final loading = context.watch<SessionsStore>().loading;
                return loading ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 2)) : const SizedBox.shrink();
              }),
            ]),
            ),
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Expanded(
                      child: Observer(builder: (_) {
                        ...
                        itemBuilder: (ctx, i) {
                            final s = sessions[i];
                            final showHeader = _groupBy != 'none' && (i == 0 || _groupKey(sessions[i-1]) != _groupKey(s));
                            final header = _groupKey(s);
                            // derive http meta or ws kind
                            final meta = (s.httpMeta ?? _httpMeta[s.id]) ?? const {};
                              ...
                              children: [
                                if (showHeader)
                                  Padding(
                                    ...
                                    child: Text(header, style: Theme.of(context).textTheme.labelMedium?.copyWith(fontWeight: FontWeight.bold)),
                                  ),
                                InkWell(
                                  onTap: () { setState(() { _selectedSessionId = s.id; }); _loadDetails(_selectedSessionId!); },
                                  child: Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8),
                                    decoration: BoxDecoration(
                                      color: _selectedSessionId == s.id ? Theme.of(context).colorScheme.primary.withOpacity(0.06) : null,
                                      borderRadius: BorderRadius.circular(6),
                                    ),
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        // URL переносим до 3 строк
                                        Text(s.target, maxLines: 3, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontFamily: 'monospace')),
                                          ...
                                          Expanded(child: Wrap(spacing: 6, runSpacing: 4, children: [
                                            ...
                                            if (!isWs && cacheStatus.isNotEmpty)
                                              (cacheStatus.toUpperCase() == 'MISS')
                                                ? _chipStrike('cache', backgroundColor: Theme.of(context).colorScheme.surfaceVariant, foregroundColor: Theme.of(context).colorScheme.onSurfaceVariant)
                                                : _chip('cache: ${cacheStatus.toUpperCase()}'),
                                            if (!isWs && !corsOk) _chip('CORS', backgroundColor: Theme.of(context).colorScheme.error.withOpacity(0.12), foregroundColor: Theme.of(context).colorScheme.error),
                                          ])),
                                          const SizedBox(width: 8),
                                          Text(
                                            _formatTimeHMSSafe(s.startedAt),
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Expanded(
                    child: Builder(builder: (context){
                      ...
                      bool selIsWs = true;
                      bool selIsHttp = true; // fallback both
                      if (_selectedSessionId != null) {
                        final items = context.watch<SessionsStore>().items.toList();
                        Map<String, dynamic>? meta;
                        String? kind;
                        for (final s in items) {
                          if (s.id == _selectedSessionId) {
                            meta = (s.httpMeta ?? _httpMeta[s.id])?.cast<String, dynamic>();
                            kind = s.kind;
                            break;
                          }
                        }
                      ...
                      return DefaultTabController(
                        length: (() {
                          if (selIsWs && selIsHttp) return 2;
                          if (selIsWs || selIsHttp) return 1;
                          return 2;
                        })(),
                        child: Column(children: [
                          Builder(builder: (context){
                            if (selIsWs && selIsHttp) {
                              return const TabBar(tabs: [Tab(text: 'WebSocket'), Tab(text: 'HTTP')]);
                            }
                          ...
                          Expanded(child: Observer(builder: (_) {
                            ...
                            final frames = details.frames.map((f)=>{
                              'id': f.id,
                            ...
                            final events = details.events.map((e)=>{
                              'id': e.id,
                          ...
                          WsDetailsPanel(
                                  ...
                                  namespaceCtrl: _namespaceFilterCtrl,
                                  onChangeOpcode: (v){ setState(() { _opcodeFilter = v; }); _savePrefs(); },
                                  onChangeDirection: (v){ setState(() { _directionFilter = v; }); _savePrefs(); },
                            hideHeartbeats: _hideHeartbeats,
                            onToggleHeartbeats: (v){ setState(() { _hideHeartbeats = v; }); _savePrefs(); },
                                ),
                                HttpDetailsPanel(sessionId: _selectedSessionId, frames: frames),
                              ]);
                            }
                            if (selIsWs) {
                            ...
                            // selIsHttp
                            return HttpDetailsPanel(sessionId: _selectedSessionId, frames: frames);
                          })),
                        ]),
                      );
                    }),
  ...
  List<dynamic> _visibleSessions() {
    ...
    final filtered = src.where((s) {
      ...
      if (_selectedRange != null) {
        ...
        final inRange = !(end != null && end.isBefore(_selectedRange!.start)) && !start.isAfter(_selectedRange!.end);
      ...
      if (_httpStatusFilter != 'any') {
        ...
        if (_httpStatusFilter == '2xx' && (st < 200 || st > 299)) return false;
        if (_httpStatusFilter == '3xx' && (st < 300 || st > 399)) return false;
        if (_httpStatusFilter == '4xx' && (st < 400 || st > 499)) return false;
        if (_httpStatusFilter == '5xx' && (st < 500 || st > 599)) return false;
      }
      ...
      if (_headerKeyCtrl.text.isNotEmpty) {
        final headers = (m['headers'] as Map?)?.map((k, v) => MapEntry(k.toString().toLowerCase(), v.toString())) ?? {};
        final hv = headers[_headerKeyCtrl.text.toLowerCase()] ?? '';
        if (_headerValCtrl.text.isNotEmpty && !hv.contains(_headerValCtrl.text)) return false;
    ...
    String keyFor(dynamic s) {
      try {
        ...
        if (_groupBy == 'route') return '${uri.host}${uri.path.split('/').take(3).join('/')}';
      } catch (_) {}
      return '';
    }
  ...
  Widget _chipStrike(String text, {Color? backgroundColor, Color? foregroundColor}) {
    final base = _chip(text, backgroundColor: backgroundColor, foregroundColor: foregroundColor);
    return Stack(
